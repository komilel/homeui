class EventsCtrl {
  constructor($rootScope, $scope, $stateParams, $state, eventsConfig, $translate, $locale, DeviceData) {
    ...
    this.eventsConfig.whenReady().then(data => {
      this.triggeredEvents = data.triggeredEvents.map(event => this.transformEvent(event, event.timestamp));
  ...
  transformEvent(event, timestamp) {
    ...
    const triggeredEvent = {
      ...
      "message": event.message,
      "timestamp": timestamp ? timestamp : this.formatTimestamp(Date.now()),
    };
